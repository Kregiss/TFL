...
Возьмём слова из этого языка вида $ba^nba^naba^{n+1}ba^{n+1}$, полученные путём пересечения изначального языка с регулярным языком $R=ba^\*ba^+ba^\*ba^*$.

Пусть L - КС язык. Тогда по лемме о накачке для КС языков существует константа накачки p.
Возьмём слово: $w = ba^pba^paba^{p+1}ba^{p+1}$.
По лемме существует такое разбиение w = xvwuy, что $|vwu| <= p$, $|vu| > 0$. Т.к. $|vwu| <= p$, то этот отрезок лежит:
1) полностью внутри блока $a^p$ или $a^{p+1}$. При i=0 нарушается взаимосвязь с блоком по другую сторону от b, т.е. с $a^p$ или $a^{p+1}$ соответственно. Слово выпадает из языка. Противоречие лемме.
2) на стыке блоков $a^p$ или $a^{p+1}$ с b или с одиночной a после второго $a^p$. При i=0 пропадает или:
   - b, т.е. поулчается слово вида
      - $w_0=a^{p-k+1}ba^paba^{p+1}ba^{p+1}$ - невозможно, т.к. слово обязано начинаться на b
      - $w_0=ba^{p-k+1}a^paba^{p+1}ba^{p+1}$ - невозможно, т.к. слово такого вида не может быть получено из данной грамматики
      - $w_0=ba^{p}ba^paa^{p+1-k+1}ba^{p+1}$ - невозможно, т.к. такое слово тоже не принадлежит данной грамматике, т.к. блок $a^p$ ожидает такой же блок после одиночной b плюс ещё одиночная разделительная a и  сразу же идёт следующий блок, который всегда начинается на b. Но дальше идут ещё a, что выбрасывает слово из языка.
      - $w_0=ba^{p}ba^paba^{p+1}a^{p+1-k+1}$ - невозможно, т.к. такое слово тоже не принадлежит данной грамматике
   - одиночная a 
      - $w_0=ba^{p}ba^{p-k+1}ba^{p+1}ba^{p+1}=ba^{p}ba^{p-k}aba^{p+1}ba^{p+1}$ - невозможно, т.к. слово принадлежит этой грамматике только при k=0.
3) в отрезке с b в середине, которая находится между двумя блоками $a^p$ или двумя блоками $a^{p+1}$ (если будем брать b между разными блоками, то нарушится их равенство с соответствующими им вторыми блоками). Пусть $w=a^tba^t, 0<=t<(p-1)/2$, $v=a^s$ (из первого блока $a^p$), $u=a^s$ (из второго блока $a^p$), $t+s=(p-1)/2$. Пусть $i=(s+1)/s$. Тогда после накачки получаем слово вида $w_0=ba^{p-s-t}a^{s+1}a^tba^ta^{s+1}a^{p-s-t}aba^{p+1}ba^{p+1}=ba^{p+1}ba^{p+1}aba^{p+1}ba^{p+1}$. А это слово не принадлежит языку, т.к. $S_1.attr = S_2.attr = 2(p+1)$, что запрещено грамматикой. Противоречие лемме. Для накачки блоков $a^{p+1}$ слово будет аналогично выпадать из языка при $i=(s-1)/s$.
   
   В любом случае слово уже не принадлежит языку. Противоречие лемме.
