...
2) на стыке блоков $a^p$ или $a^{p+2}$ с b или с одиночной a после второго $a^p$. При i=0 пропадает или:
   - b, т.е. поулчается слово вида
      - $w_0=a^{p-k+1}ba^paba^{p+2}ba^{p+2}$ - невозможно, т.к. слово обязано начинаться на b
      - $w_0=ba^{p-k+1}a^paba^{p+2}ba^{p+2}$ - невозможно, т.к. слово такого вида не может быть получено из данной грамматики
      - $w_0=ba^{p}ba^paa^{p+2-k+1}ba^{p+2}$ - невозможно, т.к. такое слово тоже не принадлежит данной грамматике, т.к. блок $a^p$ ожидает такой же блок после одиночной b плюс ещё одиночная разделительная a и  сразу же идёт следующий блок, который всегда начинается на b. Но дальше идут ещё a, что выбрасывает слово из языка.
      - $w_0=ba^{p}ba^paba^{p+2}a^{p+2-k+1}$ - невозможно, т.к. такое слово тоже не принадлежит данной грамматике
   - одиночная a 
      - $w_0=ba^{p}ba^{p-k+1}ba^{p+2}ba^{p+2}=ba^{p}ba^{p-k}aba^{p+2}ba^{p+2}$ - невозможно, т.к. слово принадлежит этой грамматике только при k=0.
3) в отрезке с b в середине, которая находится между двумя блоками $a^p$ или двумя блоками $a^{p+2}$ (если будем брать b между разными блоками, то нарушится их равенство с соответствующими им вторыми блоками). Пусть i=0. Тогда получаем слово $w_0=ba^{p-(k-1)/2}a^{p-(k-1)/2}aba^{p+2}ba^{p+2}$, т.е. слово вида $w_0=ba^ta^taba^{p+2}ba^{p+2}$, которое невозможно получить из данной грамматики, значит оно не принадлежит языку. Противоречие лемме.
